...
func init() {
    ydk.YLogDebug(fmt.Sprintf("Registering top level entities for package prm_hwmod_cfg"))
    ydk.RegisterEntity("{http://cisco.com/ns/yang/Cisco-IOS-XR-prm-hwmod-cfg hardware-module}", reflect.TypeOf(HardwareModule{}))
    ydk.RegisterEntity("Cisco-IOS-XR-prm-hwmod-cfg:hardware-module", reflect.TypeOf(HardwareModule{}))
}
...
const (
    // Urpf Disable
    UrpfConfig_disable UrpfConfig = "disable"

    // Urpf Enable
    UrpfConfig_enable UrpfConfig = "enable"
)
...
type HardwareModule struct {
    ...
    // Enabled Disabled. The type is UrpfConfig.
    Urpf interface{}
...
func (hardwareModule *HardwareModule) GetEntityData() *types.CommonEntityData {
    ...
    hardwareModule.EntityData.Leafs.Append("urpf", types.YLeaf{"Urpf", hardwareModule.Urpf})
